  ...
  validateRoleInfoForm() {
    if (this.roleForm.get('role')?.errors) {
      if (this.roleForm.get('role')?.hasError('required'))
        ...
        if(this.roleForm.get('role')?.value.charAt(0) === '-' || this.roleForm.get('role')?.value.charAt(0) === '(' || this.roleForm.get('role')?.value.charAt(0) === ')'){
          this.roleErrorMsg = "Parenthesis and hyphens are not allowed at the beginning of a role name."
        } else if(this.roleForm.get('role')?.value.slice(-1) === '-'){
          this.roleErrorMsg = "Hyphens are not allowed at the ending of a role name."
        }
        else{
          this.roleErrorMsg = 'Only alphabets,digits,parenthesis and hyphens are allowed.';
        }  
      }
        
      else if (this.roleForm.get('role')?.hasError('minlength'))
        this.roleErrorMsg = 'Please enter at least ' + this.roleForm.get('role')?.errors?.['minlength'].requiredLength + ' characters.';
      else if (this.roleForm.get('role')?.hasError('maxlength'))
        this.roleErrorMsg = 'At max ' + this.roleForm.get('role')?.errors?.['maxlength'].requiredLength + ' characters';
    } 
  }
